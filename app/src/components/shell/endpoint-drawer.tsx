...
// Inner component remounts via `key` when a new endpoint is opened,
// so tab state resets automatically without any setState-in-effect.
    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto p-5">
        ...
        {tab === "Network" && (
          <div className="flex flex-col gap-4">
            <div className="grid grid-cols-2 gap-y-3 text-[13px]">
              {([["IP", ep.ip], ["Firewall", "Corporate"], ["Connections", "12 active"]] as [string, string][]).map(([k, v]) => (
                <div key={k} className="flex flex-col gap-0.5">
                  <span className="text-[11px] font-bold uppercase tracking-[.1em]" style={{ color: "var(--ink-4)" }}>{k}</span>
        ...
        {tab === "Actions" && (
          <div className="flex flex-col gap-3">
            {[
              { label: "Isolate Endpoint", color: "var(--crit)", bg: "var(--crit-tint)", msg: "Isolation command sent", ok: true },
              { label: "Run Full Scan", color: "var(--primary)", bg: "var(--primary-tint)", msg: "Scan initiated", ok: true },
              { label: "Deploy Patches", color: "var(--ok)", bg: "var(--ok-tint)", msg: "Patch deployment queued", ok: true },
              { label: "Remote Shell", color: "var(--ink-2)", bg: "var(--surface-3)", msg: "Remote shell not available in demo", ok: false },
              { label: "Reboot", color: "var(--ink-2)", bg: "var(--surface-3)", msg: "Reboot scheduled", ok: true },
            ].map((a) => (
              <button key={a.label}
                onClick={() => addToast(a.msg, a.ok ? "success" : "default")}
...
export function EndpointDrawer() {
  ...
  return (
    <>
      ...
      >
        {/* key resets DrawerBody (and its tab state) whenever a different endpoint opens */}
